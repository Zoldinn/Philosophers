...
/*   Updated: 2025/04/29 15:36:01 by lefoffan         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	init_forks(t_fork **fork, int i, t_waiter *waiter)
{
	int	i;
...
static int	init_philos(t_philo **philo, int i, t_fork *fork, t_waiter *waiter)
{
	int	i;
	...
	while (++i < waiter->nbp)
	{
		if (pthread_create(&philo[i]->thread, NULL, philo_routine, waiter) != 0)
...
int	init(t_philo *philo, t_waiter *waiter, t_fork *fork, char **av)
{
	...
	if (init_forks(&fork[i], i, waiter) != 0)
		return (1);
	if (init_philos(&philo[i], i, fork, waiter) != 0)
		return (1);
	return (0);
}
